# abhinandati
Dansk: glæder sig ved; hæfter sig ved; finder behag i
Engelsk: delights in; takes pleasure in

## Kategori: 
tilknytning

## Kommentar
Et centralt ord i tilknytningens psykologi. Hvor der er glæde, er der binding.

## Kilder
#Bodhi #Sujato

## Forekomster
- [[majjhima/02-suttaer/MN01]]
# Referencer

#Kovilo/Nisabho:

Abhinandati [abhi + √nand + a + ti]
(from √nand, delight, enjoy)
- delights (in); is pleased (with); approves (of); is happy (with); finds pleasure (in); relishes    
- (comm) wishes (for); longs (for); desires;    
- looks forward (to) 
